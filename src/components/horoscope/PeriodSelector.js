    ...
    <div className="bg-background rounded-2xl shadow-xl p-6 border-2 border-secondary-dark">
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
        ...
        <div className="flex-1">
          <h3 className="text-lg font-bold text-main mb-3 flex items-center">
            <Calendar className="w-5 h-5 mr-2 text-primary-main" />
            Select Time Period
          </h3>
              ...
              return (
                <button
                  ...
                  className={`relative p-4 rounded-xl border-2 transition-all ${
                    isSelected
                      ? 'bg-gradient-to-br from-primary-main to-primary-light text-background border-transparent shadow-lg scale-105'
                      : 'bg-background border-secondary-dark hover:border-primary-main/40 hover:shadow-md'
                  }`}
                >
                  <div className="text-3xl mb-2">{p.icon}</div>
                  <div className={`font-bold ${isSelected ? 'text-background' : 'text-main'}`}>
                    {p.label}
                  </div>
                  <div className={`text-xs mt-1 ${isSelected ? 'text-background/80' : 'text-secondary'}`}>
                    {p.description}
                  </div>

                  {isSelected && (
                    <div className="absolute -top-2 -right-2 bg-background rounded-full p-1 shadow-lg">
                      <svg className="w-4 h-4 text-primary-main" fill="currentColor" viewBox="0 0 20 20">
              ...
              return (
                <button
                  ...
                  className={`px-4 py-3 rounded-lg font-medium transition-all ${
                    isActive
                      ? 'bg-gradient-to-r from-primary-main to-primary-light text-background shadow-lg scale-105'
                      : 'bg-primary-main/10 hover:bg-primary-main/20 text-primary-main'
                  }`}
                >
                  {selection.label}
                </button>
